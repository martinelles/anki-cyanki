<script lang="ts">
    ...
    onMount(async () => {
        try {
            if (!notebookId) { goto('/notebooks'); return; }
            const nb = await db.notebooks.get(notebookId);
            if (!nb) { alert('Caderno não encontrado.'); goto('/notebooks'); return; }
            ...
            dueCards = pool
                .filter(c => { const s = cardStates.get(c.id); return s && s.due <= now; })
                .sort((a, b) => (cardStates.get(a.id)?.due.getTime() ?? 0) - (cardStates.get(b.id)?.due.getTime() ?? 0));

            isLoading = false;
            saveSession({ type: 'notebook', id: notebookId, name: notebookTitle, cardIndex: 0, totalCards: dueCards.length, savedAt: Date.now() });
        } catch (e) {
            console.error(e);
            alert('Erro ao carregar caderno.');
            goto('/notebooks');
        }
    });
...
<div class="study-shell">
    ...
    <header class="study-header">
        <div class="header-row">
            <button on:click={finishSession} class="back-btn">
            ...
            <span class="session-label" title={notebookTitle}>{notebookTitle || '...'}</span>
...
<style>
    ...
    .criterious-toggle.active { background: rgba(99,102,241,.15); border-color: rgba(99,102,241,.4); color: #a5b4fc; }
    ...
    .suggestion-hint { font-size: 12px; font-weight: 600; color: #525252; text-align: center; margin-bottom: 8px; }
    ...
    .rating-key { font-size: 10px; font-weight: 600; font-family: inherit; opacity: .45; background: none; border: none; padding: 0; }
    ...
    .btn-primary {
        ...
        border-radius: 10px; border: none; cursor: pointer; transition: background .15s;
    }
